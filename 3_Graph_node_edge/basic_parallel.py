...
def merge_node(state: State) -> State:
    return {"input": state["input"], "output": "병합 완료", "step": 4}
...
workflow.add_edge("process", ["validate", "format"])   # fan-out
workflow.add_edge("validate", "merge")                 # fan-in
workflow.add_edge("format", "merge")                   # fan-in
